...
var Popover = (0, _react.createClass)({
  ...
  propTypes: {
    ...
    animationHandlers: _react.PropTypes.shape({
      exit: _react.PropTypes.func.isRequired,
      exitStop: _react.PropTypes.func.isRequired,
      enter: _react.PropTypes.func.isRequired,
  ...
  getDefaultProps: function getDefaultProps() {
    return {
      ...
      animationHandlers: {
        exitStop: function exitStop(popover) {
          clearTimeout(popover.exitingAnimationTimer1);
          clearTimeout(popover.exitingAnimationTimer2);
        },
        exit: function exit(popover, completionCallback) {
          popover.exitingAnimationTimer2 = setTimeout(function () {
            setTimeout(function () {
  ...
  getInitialState: function getInitialState() {
    return {
      standing: this.props.preferPlace,
  ...
  resolvePopoverLayout: function resolvePopoverLayout() {
    ...
    if(this.state.standing === 'below') {
      this.cliLayer.style.top = '5px';
    }
    if(this.state.standing === 'above') {
      this.tipEl.style.marginBottom = '-13px';
      this.cliLayer.style.top = '-16px';
    }

    if(this.state.standing === 'right') {
      this.svgContainer.style.marginRight = '-4px';
      this.svgContainer.style.marginBottom = '0';
      this.svgContainer.style.marginTop = '0';
      this.cliLayer.style.left = '14px';
      this.cliLayer.style.width = '17px';
      this.cliLayer.style.height = '110px';
      this.cliLayer.style.top = '-66px';
    }

    if(this.state.standing === 'left') {
      this.svgContainer.style.marginTop = '0';
      this.svgContainer.style.marginLeft = '-2px';
      this.cliLayer.style.right = '9px';
      this.cliLayer.style.width = '10px';
      this.cliLayer.style.height = '100px';
      this.cliLayer.style.top = '-59px';
    }
    ...
    this.cliLayer.style.transform = flowToTipTranslations[zone.flow] + '(' + tipCrossPos + 'px)';
    console.warn('this.state.standing', this.state.standing);
  },
  ...
  animateExit: function animateExit() {
    ...
    this.props.animationHandlers.exit(this, function () {
  ...
  trackPopover: function trackPopover() {
    ...
    this.cliLayer = this.containerEl.querySelector('.cli-layer');
    this.svgContainer = this.containerEl.querySelector('.svg-container');
